    ...
    try:
        if (repo_path / "pyproject.toml").exists():
            if not (repo_path / "uv.lock").exists():
                subprocess.run(['uv', 'lock'], cwd=repo_path, check=True, capture_output=True)
            subprocess.run(['uv', 'venv'], cwd=repo_path, check=True, capture_output=True)
            subprocess.run(['uv', 'sync'], cwd=repo_path, check=True, capture_output=True)
