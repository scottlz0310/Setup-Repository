...
class TestAuthenticationSecurity:
    ...
    def test_validate_session_data_valid(self):
        """有効なセッションデータの検証"""
        valid_session = {"user_id": "user123", "session_id": "session456", "created_at": "2024-01-01T00:00:00Z"}
...
class TestErrorHandlingImprovements:
    ...
    def test_error_reporter_save_report_permission_error(self):
        """権限エラー時のフォールバック"""
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 最初のopen呼び出しでPermissionError、フォールバックで成功するようにモック
            with patch("builtins.open", side_effect=[PermissionError("Permission denied"), mock_open().return_value]):
    ...
    def test_ci_error_handler_stage_error(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            handler = CIErrorHandler(enable_github_annotations=False, error_report_dir=Path(temp_dir))
