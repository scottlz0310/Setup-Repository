...
class TestSyncRepositories:
    ...
    def test_sync_repositories_dry_run_success(self, mock_mkdir, mock_ensure_uv, mock_tee_logger,
                                              mock_process_lock, mock_choose_clone_url,
                                              mock_get_repos, mock_platform_detector_class,
                                              mock_config, mock_repos):
        """ドライランモードでの成功テスト"""
        platform_info = verify_current_platform()
    ...
    def test_sync_repositories_full_success(self, mock_exists, mock_mkdir, mock_setup_python,
                                           mock_apply_vscode, mock_gitignore_manager_class,
                                           mock_sync_repo, mock_ensure_uv, mock_tee_logger,
                                           mock_process_lock, mock_choose_clone_url,
                                           mock_get_repos, mock_platform_detector_class,
                                           mock_config, mock_repos):
        """完全な同期成功テスト"""
    ...
    def test_sync_repositories_safety_check_skip(self, mock_prompt_user, mock_check_unpushed,
                                                 mock_exists, mock_ensure_uv, mock_tee_logger,
                                                 mock_process_lock, mock_choose_clone_url,
                                                 mock_get_repos, mock_platform_detector_class,
                                                 mock_config, mock_repos):
        """安全性チェックでスキップする場合"""
    ...
    @patch('src.setup_repo.sync.prompt_user_action')
    @patch('sys.exit')
    def test_sync_repositories_safety_check_quit(self, mock_exit, mock_prompt_user, mock_check_unpushed,
                                                 mock_exists, mock_ensure_uv, mock_tee_logger,
                                                 mock_process_lock, mock_choose_clone_url,
                                                 mock_get_repos, mock_platform_detector_class,
                                                 mock_config, mock_repos):
    ...
    @patch('src.setup_repo.sync.PlatformDetector')
    @patch('src.setup_repo.sync.get_repositories')
    def test_sync_repositories_test_environment(self, mock_get_repos, mock_platform_detector_class,
                                               mock_process_lock_class, mock_config):
        """テスト環境でのロック処理"""
        platform_info = verify_current_platform()
    ...
    @patch('src.setup_repo.sync.sync_repository_with_retries')
    @patch('pathlib.Path.exists')
    def test_sync_repositories_sync_error(self, mock_exists, mock_sync_repo, mock_ensure_uv,
                                         mock_tee_logger, mock_process_lock, mock_choose_clone_url,
                                         mock_get_repos, mock_platform_detector_class,
                                         mock_config, mock_repos):
        """同期エラーの場合"""
        platform_info = verify_current_platform()
