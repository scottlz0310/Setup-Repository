...
class TestUvInstaller:
    ...
    def test_uv_installer_init(self):
        ...
        # プラットフォーム情報の検証（実装に応じて調整）
        assert hasattr(installer, "platform")
        assert hasattr(installer, "shell")
    ...
    def test_custom_install_path(self):
        ...
        with patch("subprocess.run") as mock_run, patch("os.makedirs") as mock_makedirs:
