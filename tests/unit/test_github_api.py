    ...
    def test_github_api_full_workflow(self):
        ...
        with patch("requests.get") as mock_get, \
            ...
            mock_get.return_value = Mock(
                status_code=200,
                json=lambda: {"login": "testuser", "name": "Test User"}
            )
            
            # リポジトリ作成のモック
            mock_post.return_value = Mock(
                status_code=201,
                json=lambda: {"name": "test-repo", "full_name": "testuser/test-repo"}
            )
            
            # 設定更新のモック
            mock_patch.return_value = Mock(
                status_code=200,
                json=lambda: {"name": "test-repo", "description": "Updated"}
            )
            ...
            updated_repo = api.update_repository("testuser", "test-repo", 
                                                description="Updated")
