...
- **クロスプラットフォームテスト方針（重要）**:
  - GitHub Actionsでは各プラットフォーム（Windows、Linux、macOS）で実際の環境テストを実行する。
  - テストは実際の環境に応じて動作し、現在のプラットフォームに合わないテストは `pytest.skip()` でスキップする。
  - モックの使用は最小限に留め、実際のシステム呼び出しや環境依存の動作をテストする。
  - プラットフォーム固有のテストは以下のパターンを使用:
    ```python
    import platform
    if platform.system() != "Windows":
        pytest.skip("Windows環境でのみ実行")
    ```
  - 仮想的な環境をモックでテストするのではなく、実環境での動作検証を優先する。
