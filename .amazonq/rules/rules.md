...
- GitHub Actionsでは各プラットフォーム（Windows、Linux、macOS）のランナーで実際の環境テストを実行する。
- テストは実環境での動作を検証し、モックに頼らない実用的なテストを重視する。
- プラットフォーム固有の制約（fcntlモジュールなど）は適切にスキップ処理を行う。
...
- テスト条件の緩和・簡素化・スキップは一切禁止
- 「難しいので別の方法」提案は禁止
- カバレッジ向上のためだけのダミーテスト禁止
- 何度エラーが出ても諦めずに根本解決まで継続
- 意味のある検証を行わないテストは禁止
...
- レスポンシブ対応の手抜き禁止
- ブラウザ互換性問題の放置禁止
- 見た目の微調整を「細かいこと」として軽視禁止

### 【カバレッジ向上時の絶対ルール】

- test_coverage_boost.py のような名前のテストファイル作成禁止
- 単純に実装コードを呼び出すだけのテスト禁止
- __str__, __repr__ を無意味に呼ぶだけのテスト禁止
- assert文なしや常にTrueになるテスト禁止
- 各テストは実際のバグを発見できる内容であること

### 【AI作業時の基本姿勢】

- 「難しい」「複雑」「時間がかかる」等の弱音は禁止
- 3回失敗したからといって諦める行為は禁止
- エラーは解決すべき課題であり、回避理由ではない
- 妥協案の提案より根本解決を優先すること
...
## 13. Pre-commitでの修正時のコミット規約

- Pre-commitフックで自動修正された場合は、元のコミットメッセージを変更せずに `git commit --amend --no-edit` または同じメッセージで再コミットすること。
- コミットメッセージを変更すると変更内容の文脈が失われるため禁止。
- 自動修正のみの場合は「fix: Pre-commitによる自動修正」等の簡潔なメッセージを使用。

## 14. 共通スタイル

- PEP 8: 準拠を基本としたコーディングスタイルを心がける。
- 行長は原則88文字以内とし、Blackによる自動整形を前提とする。コメントおよびdocstringは72文字以内。
- PEP 257: Docstringの記述
- PEP 484: 明示的な型ヒントを付与する。`Any` の使用は例外扱いとし、理由を PR に記載する。CI で mypy を実行する。
