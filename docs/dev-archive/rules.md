...
- 既存のテストtest_*.py に対しtest_*_expanded.pyなど既存テストと重複するファイルの作成
...
- カバレッジレポート（pytest-cov の出力: coverage.xml / term-missing）を必ず確認し、未カバー行を特定して重点的にテストを追加すること
- カバレッジ向上は既存 test_*.py 内で行い、未カバー行に対応する差分を追加すること
