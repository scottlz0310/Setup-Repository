...
- スマートバージョンチェックで使用する`version-manager.py`を復旧

## [2.1.1] - 2026-01-18

### 追加

- **sync後の自動クリーンアップでスクワッシュマージ検出を選択可能**
  - `auto_cleanup_include_squash` を設定（initウィザード/設定ファイル/環境変数）
  - GitHub APIを利用するためトークンが必要
...
- **sync後の自動クリーンアップ**
  - 設定の `auto_cleanup` 有効時、同期後にマージ済みブランチを自動削除
  - initウィザードと環境変数 (`SETUP_REPO_AUTO_CLEANUP`) を追加
- **スクワッシュマージ検出の追加**
  - `cleanup --include-squash` でGitHub APIを使ったスクワッシュマージ判定に対応
